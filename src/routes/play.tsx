...
function PlayPage() {
  ...
  async function createRoom(e: React.FormEvent) {
    e.preventDefault();
    if (!partyName.trim() || !nickname.trim()) return;
    setBusy(true);
    setError("");
    try {
      ...
      const { data: room, error: rErr } = await supabase
        .from("rooms")
        .insert({ code: roomCode, name: partyName.trim().slice(0, 40), phase: "lobby" })
        .select()
        .single();
      if (rErr || !room) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center px-6 py-12">
      ...
      <p className="mt-8 text-sm font-medium text-foreground/70">Built for birthdays 🎂 and chaos</p>
    </main>
  );
}
